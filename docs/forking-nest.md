...
Search for "PMRC" in the code, and everytime it is in a string, change it for your ticker.
...
- in walletdmanager/walletdmanager.go, change the "99" and "187" by the sizes of your addresses and integrated addresses respectively:
    ...
    ```Go
    if !strings.HasPrefix(transferAddress, "PMRC") || (len(transferAddress) != 99 && len(transferAddress) != 187) {
